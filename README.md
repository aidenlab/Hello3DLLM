...
2. **Start the MCP server** (in one terminal):
   ...
   **For Netlify deployment (optional/advanced):**
   ...
   ```
   ⚠️ **Note:** When hosting the frontend on Netlify, you can use the `-u` (or `--browser-url`) parameter with your Netlify URL. This ensures that connection links generated by the MCP server point to your Netlify deployment. See [Netlify Setup](#netlify-setup-optional) for more details.
...
⚠️ **Note:** The default setup uses localhost. Netlify setup requires a WebSocket tunnel and is more complex. See [Netlify Setup](#netlify-setup-optional) for details.
...
⚠️ **Default Setup:** The default configuration uses localhost for local development. Netlify setup is optional and requires additional configuration (WebSocket tunneling). See the main [Quick Start](#quick-start) section for the default localhost setup.
